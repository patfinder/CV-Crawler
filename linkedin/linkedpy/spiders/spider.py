...
class LinkedPySpider(Spider):
    ...
    def __init__(self, mode='init', login=False):
        ...
        self.allowed_domains = ['www.linkedin.com',
                                CountryCode.code + '.linkedin.com']
    ...
    def parse_login(self, response):
        ...
        request = FormRequest.from_response(response,
                                            formname='login',
                                            formdata={
                                                'session_key': account['key'], 'session_password': account['password']},
                                            callback=self.check_login, errback=self.err_back)
        yield request

    def err_back(self, response):
        logging.error("Login failed, Try another account!!!",
                      level=logging.ERROR)
    ...
    def check_login(self, response):
        ...
        if "logout" in response.body:
            logging.info("Login successful!!!", level=logging.INFO)
            self.logged = True
            return self.run()
    ...
    def post_err_back(self, response):
        logging.info("430 Forbidden, Try another account!!!",
                     level=logging.ERROR)
        self.logged = False
        return self.run()
        ...
        while True:
            try:
                ...
                if flag:
                    ...
                    if urls == -1:
                        logging.info(
                            'No work available yet, Mission completed...', level=logging.INFO)
                        break
                    else:
                        for url in urls:
                            self.q.put(url)
    ...
    def my_parse(self, response):
        ...
        logging.info('Parsing urls from %s' % response.url, level=logging.INFO)

        # http://my.linkedin.com/directory/people/a.html
        lx1 = LinkExtractor(
            allow='(' + self.base_url + ')?' +
            r'/directory/people/([a-z]|\@)\.html',
            deny=(self.deny_re),
        )
        # http://my.linkedin.com/directory/people/my/A1.html
        lx2 = LinkExtractor(
            allow='(' + self.base_url + ')?' +
            r'/directory/people/my/[A-Z]\d+\.html',
            deny=(self.deny_re),
        )
        ...
        lx4 = LinkExtractor(
            allow='(' + self.base_url + ')?' +
            r'/pub/[a-z\-]+/[a-z0-9]+/[a-z0-9]+/[a-z0-9]+',
            deny=(self.deny_re),
        )
        ...
        try:
            ...
            links = [URL(main_url=response.url, found_urls=l1[i].url)
                     for i in range(len(l1))]
            links.extend([URL(main_url=response.url, found_urls=l2[i].url)
                          for i in range(len(l2))])
            links.extend([URL(main_url=response.url, found_urls=l3[i].url)
                          for i in range(len(l3))])
            links.extend([URL(main_url=response.url, found_urls=clean_url(
                l4[i].url)) for i in range(len(l4))])
            links.extend([URL(main_url=response.url, found_urls=clean_url(
                l5[i].url)) for i in range(len(l5))])
    ...
    def extract(self, response):
        logging.info('Extract comments from %s' % response.url, level=logging.INFO)
        response.replace(url=response.url.replace(
            'www', CountryCode.code, 1), body=response.body_as_unicode())
        ...
        p['first_name'] = hxs.select(
            '//span[@class="given-name"]/text()').extract()[0].strip()
        p['last_name'] = hxs.select(
            '//span[@class="family-name"]/text()').extract()[0].strip()
        ...
        desc_short = hxs.select(
            '//p[@class=" description summary"]/descendant-or-self::*/text()').extract()
        if desc_short:
        ...
        profile_pic = hxs.select(
            '//div[@class="photo-wrap-lg"]/a/img/@src').extract()
        if not profile_pic:
            profile_pic = hxs.select(
                '//div[@id="profile-picture"]/img/@src').extract()
        if profile_pic:
            p['profile_pic'] = profile_pic[0].strip()

        num_connection = hxs.select(
            '//div[@class="member-connections"]/strong/text()').extract()
        if not num_connection:
            num_connection = hxs.select(
                '//dd[@class="overview-connections"]/p/strong/text()').extract()

        num_connection = num_connection[0].strip().replace('+', '')
        p['num_connection'] = num_connection

        recomandations = hxs.select(
            '//dl[@id="overview"]/dd/p/strong/text()').extract()
        ...
        profile_url = hxs.select(
            '//dl[@class="public-profile"]//dd/a/@href').extract()
        if profile_url:
            profile_url = profile_url[0].strip()
            if not re.search(r'http://' + CountryCode.code, profile_url, re.M):
                logging.debug("Filted out: %s" % profile_url, level=logging.DEBUG)
                return
        else:
        ...
        email = re.search(
            r'mailto\:(\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}\b)', response.body, re.M)
        if email:
        ...
        marital_status = re.search(
            r'<dt>Marital status:</dt>\n<dd>\n<p>(\w+)</p>', response.body, re.M)
        if marital_status:
            p['marital_status'] = marital_status.group(1)

        twitter_username = re.search(
            r'http://twitter\.com/(\w+)', response.body, re.M)
        if not twitter_username:
            twitter_username = re.search(
                r'/redirect\?url=http%3A%2F%2Fwww%2Etwitter%2Ecom%2F(\w+)\&', response.body, re.M)
        if twitter_username:
        ...
        # education
        edu_list = []
        vcalendar = hxs.select(
            '//div[@id="profile-education"]//div[@class="content vcalendar"]/div/div')
        for vcard in vcalendar:
            edu = Education()
            date_start = vcard.select(
                './/abbr[@class="dtstart"]/text()').extract()
            if date_start:
            ...
            organization = vcard.select(
                './/h3[@class="summary fn org"]/a/text()').extract()
            if not organization:
                organization = vcard.select(
                    './/h3[@class="summary fn org"]/text()').extract()
        ...
        if skills_table:
            skill_rows = skills_table.select(
                'li[@class="competency show-bean  "]/span/a/text()').extract()
            ...
            skills_rows = skills_table.select(
                'li[@class="competency show-bean  extra-skill"]/span/a/text()').extract()
            if skills:
                for skills in skill_rows:
                    skill = skills.split(',')
                    for s in skill:
        ...
        if not skills_list:
            ...
            if skills:
                for i in range(len(skills)):
                    ski = skills[i].select(
                        './/span[@class="endorse-item-name-text"]/text()').extract()
                    no_endorsements = skills[i].select(
                        './/span[@class="num-endorsements"]/text()').extract()
                    s = Skill()
            ...
            skills = hxs.select(
                '//ol[@class="skills-with-endorsements compact-view"]/li')
            if skills:
                for i in range(len(skills)):
                    ski = skills[i].select(
                        './/span[@class="endorse-item-name-text"]/text()').extract()
        ...
        # specialities
        specialities_list = []
        specialties = hxs.select(
            '//div[@id="profile-specialties"]//p/text()').extract()
        if specialties:
        ...
        experience_list = []
        vcalendar = hxs.select(
            '//div[@id="profile-experience"]//div[@class="content vcalendar"]/div/div/div')
        ...
        for vcard in vcalendar:
            ...
            if not title:
                title = vcard.select(
                    './/strong[@class="title"]/a/text()').extract()
            ...
            if title:
                ...
                date_end = vcard.select(
                    './/abbr[@class="dtend"]/text()').extract()
                if date_end:
                    exp['date_end'] = date_end[0].strip()

                description = vcard.select(
                    './/p[@class=" description current-position"]/descendant-or-self::*/text()').extract()
                if not description:
                    description = vcard.select(
                        './/p[@class=" description past-position"]/descendant-or-self::*/text()').extract()
                if description:
                ...
                organization = vcard.select(
                    './/span[@class="org summary"]/text()').extract()
                if not organization:
                    organization = vcard.select(
                        './/h4/strong/a[@name="company"]/text()').extract()

                if organization:
                    exp['organization'] = organization[0].strip(
                    ).replace('&amp;', '&')
        ...
        for group in groups:
            g = Group()
            g['group_url'] = 'http://www.linkedin.com' + \
                group.select('.//a/@href').extract()[0]
            g['organization'] = group.select(
                './/a/strong[@class="fn org"]/text()').extract()[0].strip().replace('&amp;', '&')
