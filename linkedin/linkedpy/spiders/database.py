...
class Database(object):
    ...
    def feed(self, mode):
        ...
        while True:
            if random:
                cursor.execute(
                    'select found_urls from crawler_urls use index(stat) where stat = %s order by rand() limit %s , %s', (status, self.offset, limit))
            else:
                cursor.execute(
                    'select found_urls from crawler_urls use index(stat) where stat = %s limit %s , %s', (status, self.offset, limit))
            ...
            if rows:
                ...
                cursor.execute(
                    'select count(*) as quantity from crawler_urls use index(stat) where stat = %s', (status))
    ...
    def insert_urls(self, items):
        ...
        for item in items:
            if item['found_urls'] == '$':
                cursor.execute(
                    "update crawler_urls use index(found_urls) set stat = 'C' where found_urls = %s limit 1", (item['main_url'], ))
                cursor.execute('commit')
            else:
                cursor.execute(
                    "select sno, stat from crawler_urls use index(found_urls) where found_urls = %s limit 1", (item['found_urls'], ))
                result = cursor.fetchone()

                if not result:  # existed in crawler_urls
                    cursor.execute("insert into crawler_urls (main_url, found_urls) values (%s, %s)", (
                        item['main_url'], item['found_urls']))
                    logging.debug(" - [W] %s" %
                                  item['found_urls'], level=logging.DEBUG)
        cursor.execute('commit')
    ...
    def insert_profile(self, item):
        ...
        # udpate crawled_url
        cursor.execute(
            'select sno from crawled_urls use index(url) where url = %s limit 1', (item['profile_url']))
        result = cursor.fetchone()
        if result:
            crawled_urls_sno = result['sno']
        else:
            cursor.execute(
                "insert into crawled_urls (url) values (%s)", (item['profile_url']))
            logging.debug(" - [P] %s" %
                          item['profile_url'], level=logging.DEBUG)
            crawled_urls_sno = cursor.lastrowid

        cursor.execute(
            'select sno from linkedin_profiles use index(profile_url) where profile_url = %s limit 1', (item['profile_url']))
        result = cursor.fetchone()
        if not result:
            ...
            cursor.execute(
                ...
                (
                    crawled_urls_sno, item.get(
                        'profile_url'), item.get('title'),
                    item.get('first_name'), item.get(
                        'last_name'), item.get('locality'),
                    item.get('region'), item.get(
                        'country'), item.get('desc_short'),
                    item.get('profile_pic'), item.get(
                        'num_connection'), item.get('email'),
                    item.get('phone'), item.get(
                        'twitter_username'), item.get('department'),
                    item.get('recomandations'),
                    item.get('im'), item.get(
                        'address'), item.get('birthday'),
            ...
            for edu in item['education']:
                cursor.execute(
                    'insert into linkedin_education (profile_sno, date_start, date_end, degree, organization)\
                        values (%s,%s,%s,%s,%s)',
                    (
                        profile_sno, self.string_to_date(
                            edu.get('date_start')), self.string_to_date(edu.get('date_end')),
            ...
            for exp in item['experience']:
                cursor.execute(
                    ...
                    (
                        profile_sno, self.string_to_date(
                            exp.get('date_start')), self.string_to_date(exp.get('date_end')),
                        exp.get('title'), exp.get(
                            'organization'), exp.get('description')
                    )
                )
            # skills
            ...
            cursor.execute('commit')
            logging.info(" - [Added] profile: %s" %
                         item['profile_url'], level=logging.INFO)
    ...
    def update_profile(self, item):
        ...
        # education
        cursor.execute(
            'delete from linkedin_education where profile_sno = %s', profile_sno)
        for edu in item['education']:
            cursor.execute(
                'insert into linkedin_education (profile_sno, date_start, date_end, degree, organization)\
                    values (%s,%s,%s,%s,%s)',
        ...
        # experience
        cursor.execute(
            'delete from linkedin_experience where profile_sno = %s', profile_sno)
        for exp in item['experience']:
            cursor.execute(
                'insert into linkedin_experience (profile_sno, date_start, date_end, title, organization, description)\
                    values (%s,%s,%s,%s,%s, %s)',
        ...
        # skills
        cursor.execute(
            'delete from linkedin_skills where profile_sno = %s', profile_sno)
        for ski in item['skills']:
            cursor.execute(
                'insert into linkedin_skills (profile_sno, skill, no_endorsements, first_skill_ind)\
                    values (%s,%s, %s, %s)',
        ...
        # specialties
        cursor.execute(
            'delete from linkedin_specialities where profile_sno = %s', profile_sno)
        for spe in item['specialties']:
            cursor.execute(
                'insert into linkedin_specialities (profile_sno, specialty)\
                    values (%s,%s)',
        ...
        # websites
        cursor.execute(
            'delete from linkedin_websites where profile_sno = %s', profile_sno)
        for w in item['websites']:
            cursor.execute(
                'insert into linkedin_websites (profile_sno, website, cate)\
                    values (%s,%s, %s)',
        ...
        # interests
        cursor.execute(
            'delete from linkedin_interests where profile_sno = %s', profile_sno)
        for w in item['interests']:
            cursor.execute(
                'insert into linkedin_interests (profile_sno, interest)\
                    values (%s,%s)',
        ...
        # groups
        cursor.execute(
            'delete from linkedin_groups where profile_sno = %s', profile_sno)
        for w in item['groups']:
            cursor.execute(
                'insert into linkedin_groups (profile_sno, group_url, organization)\
                    values (%s,%s,%s)',
        ...
        # honors
        cursor.execute(
            'delete from linkedin_honors where profile_sno = %s', profile_sno)
        for w in item['honors']:
            cursor.execute(
                'insert into linkedin_honors (profile_sno, honor)\
                    values (%s,%s)',
        ...
        logging.debug(" - [Updated] crawled_urls: %s" %
                      item['profile_url'], level=logging.DEBUG)
